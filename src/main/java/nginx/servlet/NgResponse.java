...
public class NgResponse implements HttpServletResponse {
	...
	public NgResponse(MemorySegment request) {
		this.request = request;
	}

	@Override
	public String getCharacterEncoding() {
		long len = (long) vhCharsetTypeLen.get(request, 0L);
		if ( len == 0 ) return null;
		MemorySegment dataSeg = (MemorySegment) vhCharsetTypeData.get(request, 0L);
	...
	private VarHandle vhPool = NgHttpRequest.ngx_http_request_t.varHandle(
		groupElement("pool")
	);
	...
	private final static VarHandle 
	    vhCharsetTypeLen = NgHttpRequest.ngx_http_request_t.varHandle(
			groupElement("headers_out"),
			groupElement("charset"),
			groupElement("len")
		),
	    vhCharsetTypeData = NgHttpRequest.ngx_http_request_t.varHandle(
			groupElement("headers_out"),
			groupElement("charset"),
			groupElement("data")
		);
	...
	public void setCharacterEncoding(String encoding) {
		if ( encoding != null && encoding.length() > 0 ) {
			...
			vhCharsetTypeData.set(request, 0L, seg);
			vhCharsetTypeLen.set(request, 0L, encBytes.length);
		}
	}

	private final static VarHandle vhContentLength = NgHttpRequest.ngx_http_request_t.varHandle(
		groupElement("headers_out"),
	...
	public void setContentLengthLong(long len) {
		vhContentLength.set(request, 0L, (long) len);
	}

	private final static VarHandle 
		vhContentTypeLen = NgHttpRequest.ngx_http_request_t.varHandle(
			groupElement("headers_out"),
			groupElement("content_type"),
			groupElement("len")
		),
		vhContentTypeData = NgHttpRequest.ngx_http_request_t.varHandle(
			groupElement("headers_out"),
			groupElement("content_type"),
			groupElement("data")
		),
		vhContentType_Len = NgHttpRequest.ngx_http_request_t.varHandle(
			groupElement("headers_out"),
	...
	public void setContentType(String type) {
		...
		if ( type != null && type.length() > 0 ) {
			...
			vhContentTypeData.set(request, 0L, seg);
		}
		
		vhContentTypeLen.set(request, 0L, len);
		vhContentType_Len.set(request, 0L, len);
	}
	...
	public void addHeader(String name, String value) {
		...
		NgString.vhStrLen.set(keySeg, 0L, nl);
		NgString.vhStrData.set(keySeg, 0L, kv);
		
		MemorySegment valSeg = tableEltSeg.asSlice(NgHash.offsetValue, NgString.ngx_str_t.byteSize());
		NgString.vhStrLen.set(valSeg, 0L, vl);
		NgString.vhStrData.set(valSeg, 0L, kv.asSlice(nl));
