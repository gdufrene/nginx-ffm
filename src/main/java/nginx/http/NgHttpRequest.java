...
public interface NgHttpRequest extends NgGlobal {
	...
	MethodHandle 
		  ngx_http_output_filter = downcall("ngx_http_output_filter", of(JAVA_INT, ADDRESS, ADDRESS))
		, ngx_http_send_header = downcall("ngx_http_send_header", of( ValueLayout.JAVA_INT, ValueLayout.ADDRESS ))
		, ngx_http_discard_request_body = downcall("ngx_http_discard_request_body", of( JAVA_INT, ADDRESS ))
		;
	
	VarHandle
		vhPool = ngx_http_request_t.varHandle(PathElement.groupElement("pool")),
		vhResponseStatus = ngx_http_request_t.varHandle(
			PathElement.groupElement("headers_out"),
			PathElement.groupElement("status")
		),
		vhResponseContentLength = ngx_http_request_t.varHandle(
			PathElement.groupElement("headers_out"),
